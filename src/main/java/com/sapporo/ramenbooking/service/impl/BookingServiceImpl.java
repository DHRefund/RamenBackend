...
import com.sapporo.ramenbooking.repository.*;
...
public class BookingServiceImpl implements BookingService {
        ...
        public BookingResponse createBooking(BookingRequest request) {
                ...
                TimeSlot timeSlot = timeSlotRepository.findById(request.getTimeSlotId())
                                .orElseThrow(() -> new BookingException("Khung giờ không tồn tại"));

                // if (!timeSlot.getAvailable() ||
                // timeSlot.getBookedCount() >= timeSlot.getMaxCapacity()) {
                // throw new BookingException("Khung giờ đã hết chỗ");
                // }
                // Atomic: check + update trong 1 câu lệnh
                int rowsAffected = timeSlotRepository.bookIfAvailable(request.getTimeSlotId());
                if (rowsAffected == 0) {
                        throw new BookingException("Khung giờ đã hết chỗ");
                }
        ...
        public List<BookingResponse> getAllReservationsByDate(LocalDate date) {
                return reservationRepository.findByBookingDateOrderByTimeSlotAsc(date).stream()
                                .map(r -> BookingResponse.builder()
                                                .bookingId(r.getId())
                                                .status(r.getStatus().name())
                                                .createdAt(r.getCreatedAt())
        ...
        public BookingResponse cancelReservation(String reservationId) {
                ...
                if (reservation.getStatus() == ReservationStatus.CANCELLED) {
                        throw new BookingException("Đặt chỗ đã bị hủy trước đó");
                }
                ...
                TimeSlot timeSlot = timeSlotRepository.findByDateAndTime(
                                ...
                                reservation.getTimeSlot()).orElse(null);
        ...
        public List<BookingResponse> findReservationsByPhone(String customerPhone) {
                List<Reservation> reservations = reservationRepository
                                .findByCustomerPhoneAndStatusNot(customerPhone, ReservationStatus.CANCELLED);
        ...
        @Override
        @Transactional
        public CancelBookingResponse cancelBookingByCustomer(CancelBookingRequest request) {
                Reservation reservation = reservationRepository.findById(request.getBookingId())
                                .orElseThrow(() -> new BookingException("Không tìm thấy đặt chỗ"));

                if (!reservation.getCustomerPhone().equals(request.getCustomerPhone())) {
                        throw new BookingException("Số điện thoại không khớp với chủ đặt chỗ");
                }

                if (reservation.getStatus() != ReservationStatus.BOOKED) {
                        throw new BookingException("Đặt chỗ này không thể hủy (đã hủy hoặc đã hoàn thành)");
                }
                ...
                long hoursUntilBooking = Duration.between(LocalDateTime.now(), bookingDateTime).toHours();

                if (hoursUntilBooking < 2) {
                        throw new BookingException("Chỉ có thể hủy trước giờ đặt ít nhất 2 giờ");
                }

                reservation.setStatus(ReservationStatus.CANCELLED);
                reservationRepository.save(reservation);

                timeSlotRepository.findByDateAndTime(reservation.getBookingDate(), reservation.getTimeSlot())
                                .ifPresent(timeSlot -> {
                                        timeSlot.setBookedCount(Math.max(0, timeSlot.getBookedCount() - 1));
                                        timeSlot.setAvailable(true);
                                        timeSlotRepository.save(timeSlot);
        ...
        private void validateBookingDate(LocalDate date) {
                ...
                if (date.isBefore(today)) {
                        throw new BookingException("Không thể đặt ngày trong quá khứ");
                }
                if (date.isAfter(maxDate)) {
                        throw new BookingException("Chỉ có thể đặt trước tối đa 30 ngày");
                }
        }

        private List<TimeSlot> getOrCreateTimeSlots(LocalDate date) {
                List<TimeSlot> slots = timeSlotRepository.findByDateOrderByTimeAsc(date);
        ...
        private List<TimeSlot> createDefaultTimeSlots(LocalDate date) {
                String[] times = { "11:00", "11:30", "12:00", "12:30", "13:00", "13:30",
        ...
        public BookingDetailResponse getBookingById(String bookingId) {
                Reservation reservation = reservationRepository.findById(bookingId)
                                .orElseThrow(() -> new BookingException("Không tìm thấy đặt chỗ"));
                return BookingDetailResponse.builder().bookingId(reservation.getId())
                                .customerName(reservation.getCustomerName())
                                .customerPhone(reservation.getCustomerPhone())
                                .customerEmail(reservation.getCustomerEmail()).bookingDate(reservation.getBookingDate())
                                .timeSlot(reservation.getTimeSlot()).numberOfGuests(reservation.getNumberOfGuests())
                                .status(reservation.getStatus().name()).createdAt(reservation.getCreatedAt())
                                .updatedAt(reservation.getUpdatedAt()).build();
        }
